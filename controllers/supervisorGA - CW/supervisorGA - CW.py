...
class SupervisorGA:
    def __init__(self):
        ...
        # Variable for reaching the reward
        ...
        print(self.display, self.width, self.height)
    ...
    def evaluate_genotype(self,genotype,generation):
        ...
        while currentInteraction < numberofInteractionLoops:
            ...
            # Get final coordinates of the robot
            self.final_x = self.robot_node.getField("translation").getSFVec3f()[0]
            self.final_z = self.robot_node.getField("translation").getSFVec3f()[2]
            # Measure fitness
            fitness = self.receivedFitness
            #print("combined fitness :" + str(fitness))
            
            # Check for Reward and add it to the fitness value here
            if self.final_x >= 0.3 and self.final_x <= 0.4 and self.final_z >= -0.2 and self.final_z <= -0.1:
                self.reward_reached = True
                print("reward reached")
            else:
                self.reward_reached = False
                
            if self.reward_reached == True:
                fitness *= 1.6
            else:
                # Influencing the fitness by how far the robot has come from its starting position
                self.d_final = math.sqrt((self.final_x - 0.35)**2 + (self.final_z + 0.15)**2)
                #print("d :" + str(self.d_final))
                if self.final_x >= -0.1 and self.final_x <= 0.1 and self.final_z >= -0.2 and self.final_z <= -0.1:
                    print("corner blocked")
                    fitness *= 0.7
                else:
                    if self.d_final < 0.2:
                        fitness *=1.4
                    elif self.d_final < 0.5:
                        fitness *=1.2
                    elif self.d_final > 0.55:
                        fitness *= 0.4
            if fitness > 1:
                fitness = 1
            ...
            # Get final coordinates of the robot
            final_x = self.robot_node.getField("translation").getSFVec3f()[0]
            final_z = self.robot_node.getField("translation").getSFVec3f()[2]
            ...
            # Check for Reward and add it to the fitness value here
            if final_x >= -0.4 and final_x <= -0.3 and final_z >= -0.2 and final_z <= -0.1:
                self.reward_reached = True
                print("reward reached")
            else:
                self.reward_reached = False
